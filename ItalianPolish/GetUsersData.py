...
def save_tweet(mark, connection, data_dict, keyword):
	...
	if results != []:
		...
		if existing_keyword_count is not None:
			sql = "UPDATE tweets SET keywords [" + str(existing_keyword_count + 1) + "] = '" + keyword + "' WHERE id = " + str(results[0][0])
			# return False
	else:
...
def get_existing_id(mode, mark, keyword):
	...
	sql = "SELECT id FROM tweets WHERE '" + keyword.lower() + "' = ANY(keywords) ORDER BY id " + sort_direction + " LIMIT 1"
	# print(sql)
	# pdb.set_trace()
	mark.execute(sql)
	results = mark.fetchall()
...
def get_tweets_containing_keyword(keyword, max_id, since_id, auth, mark, connection):
	last_tweet_saved_id = ''
	try:
		# pdb.set_trace()
		...
		for j, page in enumerate(c.pages()):
			...
			for tweet in page:
				...
				last_tweet_saved_id = tweet_data_dict['id']				
...
for settings_file in settings_file_list:
	...
	for keyword in keywords:
		...
		get_tweets_containing_keyword(keyword, False, newest_id, auth, mark, connection)
